...
class OsticketConfig extends Config {
    ...
    function getSocialLinks() {
        return array_filter(array(
            'facebook' => trim($this->get('facebook_url')),
            'x' => trim($this->get('x_url')),
            'instagram' => trim($this->get('instagram_url')),
            'linkedin' => trim($this->get('linkedin_url')),
        ));
    }
    ...
    function getSocialFooterText() {
        ...
        foreach ($links as $name => $url) {
            $parts[] = sprintf('%s: %s', $labels[$name] ?? ucfirst($name), $url);
        }

        return "Follow us on:\n" . implode("\n", $parts);
    }

    function getSocialFooterHtml() {
        $links = $this->getSocialLinks();
        if (!$links)
            return '';

        $icons = array(
            'facebook' => array('label' => 'Facebook', 'glyph' => 'f'),
            'x' => array('label' => 'X', 'glyph' => 'x'),
            'instagram' => array('label' => 'Instagram', 'glyph' => 'ig'),
            'linkedin' => array('label' => 'LinkedIn', 'glyph' => 'in'),
        );
        ...
        foreach ($links as $name => $url) {
            ...
            $info = $icons[$name];
            $buttons[] = sprintf(
                '<a href="%1$s" target="_blank" rel="noopener noreferrer" aria-label="%3$s" title="%3$s" style="display:inline-block;margin:0 4px;width:34px;height:34px;line-height:34px;text-align:center;border-radius:999px;border:1px solid #8f8f8f;color:#f5f5f5;font-family:Arial,sans-serif;font-size:14px;font-weight:700;background:#2b2b2b;text-decoration:none;text-transform:uppercase;">%2$s</a>',
        ...
        return '<div style="margin-top:24px;padding-top:18px;border-top:1px solid #3a3a3a;text-align:center;">'
            .'<div style="margin-bottom:10px;color:#b9b9b9;font-family:Arial,sans-serif;font-size:12px;line-height:1.4;">'
            .__('Follow us on:').'</div>'
    ...
    function updateEmailsSettings($vars, &$errors) {
        ...
        foreach (array('facebook_url', 'x_url', 'instagram_url', 'linkedin_url') as $key) {
            $value = trim($vars[$key] ?? '');
        ...
        return $this->updateAll(array(
            ...
            'facebook_url'=>trim($vars['facebook_url'] ?? ''),
            'x_url'=>trim($vars['x_url'] ?? ''),
            'instagram_url'=>trim($vars['instagram_url'] ?? ''),
            'linkedin_url'=>trim($vars['linkedin_url'] ?? ''),
         ));
    }
