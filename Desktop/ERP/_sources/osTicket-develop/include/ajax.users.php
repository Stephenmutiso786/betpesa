...
class UsersAjaxAPI extends AjaxController {
    ...
    function emailUser($id) {
        ...
        elseif (!($user = User::lookup($id)))
            Http::response(404, 'Unknown user');
        elseif (!($user_email = trim((string) $user->getEmail())))
            Http::response(400, __('This user does not have an email address on file'));

        $errors = array();
        $info = array(
            'to' => $user_email,
            'subject' => Format::htmlchars($_POST['subject'] ?? ''),
            'message' => Format::htmlchars($_POST['message'] ?? ''),
            'title' => sprintf(__('Email %s'), Format::htmlchars($user->getName())),
        );
        ...
        if ($_POST) {
            ...
            if (!$errors) {
                $email = $cfg ? $cfg->getDefaultEmail() : null;
                $from = $email ? $email->getAddress() : null;
                $fromName = $email ? $email->getName() : null;
                $options = array('text' => true);
                if ($fromName)
                    $options['from_name'] = $fromName;

                $sent = $email
                    ? $email->sendAlert($user_email, $subject, $message, null, $options)
                    : \osTicket\Mail\Mailer::sendmail($user_email, $subject, $message, $from, $options);

                if ($sent) {
                    Http::response(201, __('Email sent successfully'));
                }

                $errors['err'] = __('Unable to send email. Please try again.');
            }

            $info['subject'] = Format::htmlchars($_POST['subject']);
            $info['message'] = Format::htmlchars($_POST['message']);
        }

        include(STAFFINC_DIR . 'templates/user-email.tmpl.php');
    }
