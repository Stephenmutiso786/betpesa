...
implements RestrictedAccess, Threadable, Searchable {
    ...
    function setStatus($status, $comments='', &$errors=array(), $set_closing_agent=true, $force_close=false) {
        ...
        if ($status->getState() == 'closed' && $hadStatus) {
            $owner = $this->getOwner();
            $dept = $this->getDept();
            $email = $dept ? $dept->getAutoRespEmail() : null;
            if ($owner && (!$email && $cfg))
                $email = $cfg->getDefaultEmail();
            if ($owner && $email) {
                $signature = $dept ? $dept->getSignature() : '';
                $subject = sprintf(__('%s [#%s]'), $this->getSubject(), $this->getNumber());
                $body = sprintf(
                    ...
                    .'%s<br>%s</div>',
                    sprintf(__('Dear %s,'), Format::htmlchars($owner->getName())),
                    sprintf(__('Your support request #%s has been resolved and closed.'), $this->getNumber()),
                    $owner->getTicketLink(),
                    __('View your ticket online'),
                    __('If you still need help, reply to this email and the ticket will be reopened for review.'),
                    $signature
                );
                $email->sendAutoReply($owner, $subject, $body, null, array(
